...
public:
  ...
  void GetScreenBitmap(std::string WindowName) {

    ::EnumWindows(
        [](HWND hwnd, LPARAM lParam) -> BOOL {
          LPTSTR lpString = (LPTSTR)VirtualAlloc((LPVOID)NULL, (DWORD)(1000 + 1), MEM_COMMIT, PAGE_READWRITE);
		  DWORD dwProcessId;
		  ::GetWindowTextW(hwnd, lpString, 1000);
          std::wstring sWindowText = std::wstring((wchar_t *)(lpString));
          std::wcout << sWindowText << L"\r\n";
          const std::wregex re_chrome(_T("(Minefield, HTML5 Massively Multiplayer Online Minesweeper)"));
          std::wsmatch match;
  ...
  int CaptureAnImage(HWND hWnd) {
    ...
    hbmScreen =
        CreateCompatibleBitmap(hdcWindow, rcClient.right - rcClient.left,
